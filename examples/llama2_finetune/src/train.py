...
def create_model(cfg, trainer):
    ...
    # Non-invasive quantization enablement via Transformer Light
    if cfg.model.fp8:
        config = QuantConfig(format=QuantFormat.FP8_E4M3,
                             scaling=ScalingType.DELAYED)
        quant.enable(model, config=config)

    # Determine attention backend: ck (AITER) or triton
    custom = cfg.model.get("custom", {})
    prefer = custom.get("attention_backend", "auto")
    backend = quant.get_attention_backend(prefer=prefer)
    logging.info(f"Transformer Light attention backend: {backend} "
                 f"(AITER available: {quant.is_aiter_available()})")
