...
> **本报告复现了 FlyDSL 卷积算子的性能提升，并验证该收益能够真实传导到完整 VAE
> encode：Qwen-Image 提升 1.92×，Wan2.1 提升 1.27×。** 但 VAE 只是当前训练 step
> 的一部分，按实测占比推导的训练端到端收益仅约 0.40% 和 2.05%，均未超过重复运行
> 波动。因此，当前结果应表述为“VAE 阶段加速已经成立”，而不是“训练端到端加速
> 已经成立”。

| 结论层级 | 结论 | 证据 |
|---|---|---|
| 卷积算子 | **性能提升已复现** | Qwen 1024 卷积最高 4.97×；Wan 81 帧真实 T>1/cache Conv3d 为 1.59× |
| 完整 VAE | **算子收益确实带来 VAE 加速** | Qwen encode `33.05 → 17.22 ms`（1.92×）；Wan encode `705.8 → 556.1 ms`（1.27×） |
| 训练端到端 | **提升不明显，当前不能主张稳定加速** | 理论节省仅 0.40% / 2.05%，小于约 ±6% / 2.2–4.3% 的重复运行波动 |

这一区分很重要：VAE 内部的收益没有丢失。Wan 的卷积时间减少 153.04 ms，按 VAE
内部占比推导出的完整 encode 约为 553 ms，实测 556.1 ms，基本吻合。端到端不明显的
原因是 VAE 之外还有 DiT 前后向、优化器和分布式通信，而不是卷积优化没有生效。

因此该优化更适合 VAE 占比较高的预处理、latent 生成或独立 VAE 服务。完整生成推理
还需要单独测量 VAE decode；本文只验证了 encode，不能直接把 encode 加速比外推到
T2I/T2V 的完整推理延迟。
...
1. FlyDSL 内核确实进入了 8 卡训练和离线 embedding 的执行路径，不是静默回退；
   卷积算子的性能提升已在真实模型 shape 上复现。
2. 算子收益确实传导到了完整 VAE，Qwen-Image 和 Wan 的 encode 分别提升 1.92× 和
   1.27×，并非只有脱离模型的 microbenchmark 变快。
3. Qwen-Image 和 Wan 的 BF16 encode 精度相对 FP32 均未劣化。
4. 当前训练配置下，FlyDSL 的独立 step-time 收益小于运行噪声；VAE 加速成立，但不能
   据此报告稳定的训练端到端加速比。
