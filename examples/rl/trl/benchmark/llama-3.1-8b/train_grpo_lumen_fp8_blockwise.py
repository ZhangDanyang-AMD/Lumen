"""Apple-to-apple FP8 run for Llama-3.1-8B (blockwise scaling).
...
This should produce lower quantization error than dynamic scaling.

Launch:
  accelerate launch --config_file ddp_8gpu.yaml \
    --num_processes 8 --main_process_port 29500 \
    examples/rl/trl/benchmark/llama-3.1-8b/train_grpo_lumen_fp8_blockwise.py --max-steps 1000
"""
...
OUTPUT_DIR = os.environ.get(
    "OUTPUT_DIR",
    "/workspace/Lumen/outputs/benchmark/llama-3.1-8b/lumen_fp8_blockwise",
)
...
def main():
    ...
    print(f"[fp8_blockwise] Model={MODEL_PATH}  GPUs={num_gpus}  per_device_bs={per_device_bs}  "
