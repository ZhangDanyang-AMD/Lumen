"""BF16 baseline GRPO training for Qwen2.5-32B-Instruct (FSDP sharded).

Launch:
  accelerate launch --config_file fsdp_8gpu.yaml \
    --num_processes 8 --main_process_port 29505 \
...
OUTPUT_DIR = os.environ.get(
    "OUTPUT_DIR",
    "/workspace/Lumen/outputs/benchmark/qwen2.5-32b-instruct/bf16_baseline",
)
...
class _PerfLogCallback(TrainerCallback):
    ...
    def on_log(self, args, state, control, logs=None, **kwargs):
        ...
        print(
            f"[bf16_32b] Step {state.global_step}: "
    ...
    def on_train_end(self, args, state, control, **kwargs):
        if state.is_world_process_zero and self._step_times:
            ...
            print(
                f"[bf16_32b] Avg step time: {avg:.2f}s over {len(self._step_times)} steps",
                flush=True,
            )
...
def main():
    ...
    print(
        f"[bf16_32b] Rank={rank}  Model={MODEL_PATH}  GPUs={num_gpus}  "
    ...
    if torch.cuda.is_available():
        ...
        free, total = torch.cuda.mem_get_info()
        print(
            f"[bf16_32b] Rank={rank} Pre-training: peak_mem={pre_mem:.2f} GB  "
    ...
    if torch.cuda.is_available() and rank == 0:
        ...
        print(f"[bf16_32b] Final peak GPU memory: {peak_gb:.2f} GB", flush=True)
    if rank == 0:
        print("[bf16_32b] BF16 baseline training COMPLETE!", flush=True)
