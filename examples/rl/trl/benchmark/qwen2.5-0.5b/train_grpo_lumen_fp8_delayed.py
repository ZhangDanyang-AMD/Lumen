...
amax every forward pass) to measure the accuracy/speed trade-off.

Launch:
  OUTPUT_DIR=... accelerate launch --config_file ddp_8gpu.yaml \
    --num_processes 8 --main_process_port 29500 \
    examples/rl/trl/benchmark/qwen2.5-0.5b/train_grpo_lumen_fp8_delayed.py --max-steps 1000
"""
...
OUTPUT_DIR = os.environ.get(
    "OUTPUT_DIR",
    "/workspace/Lumen/outputs/benchmark/qwen2-0.5b-exact/lumen_fp8_delayed",
)
...
def _build_fp8_model():
    """Load model in BF16, apply FP8 delayed-scaling hooks."""
...
def main():
    ...
    print(f"[fp8_delayed] GPUs={num_gpus}  per_device_bs={per_device_bs}  "
          f"grad_accum={grad_accum}  effective_batch={num_gpus * per_device_bs * grad_accum}")
    print("[fp8_delayed] FP8 training enabled: fp8_e4m3, DELAYED scaling (TE-style history)")
    ...
    if torch.cuda.is_available():
        ...
        print(f"[fp8_delayed] Peak GPU memory: {peak_gb:.2f} GB")
    print("FP8 delayed-scaling training done!")
