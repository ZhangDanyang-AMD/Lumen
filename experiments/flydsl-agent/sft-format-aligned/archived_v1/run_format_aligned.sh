#!/bin/bash
# Launch Format-Aligned SFT training (plan.md §Stage 0).
# Base model: v5e merged weights. LoRA r=32, 1 epoch, lr 5e-6.
set -euo pipefail
...
CONTAINER_NAME="${CONTAINER_NAME:-lumen_format_aligned_sft}"
LOG_NAME="auto_format_aligned.log"

mkdir -p "${HOST_RESULTS}"
source "${SCRIPT_DIR}/config_format_aligned.sh"

# Compute max steps: 1 epoch over training data
TRAIN_FILE="${HOST_DATA}/data/format_aligned/train.jsonl"
if [ ! -f "${TRAIN_FILE}" ]; then
    echo "ERROR: Training data not found at ${TRAIN_FILE}"
    echo "Run generate_format_data.py first."
    exit 1
fi
NUM_SAMPLES=$(wc -l < "${TRAIN_FILE}")
GBS=$((MBS * GRAD_ACCUM * NGPU))
MAX_STEPS=$(( (NUM_SAMPLES + GBS - 1) / GBS ))  # 1 epoch
LR_WARMUP_STEPS=$(( MAX_STEPS / 10 ))  # 10% warmup

echo "================================================================"
echo " FlyDSL Agent — Format-Aligned SFT (Stage 0)"
...
echo "  Steps:       ${MAX_STEPS} (1 epoch), warmup ${LR_WARMUP_STEPS}"
echo "  LR:          ${LR}"
echo "================================================================"
...
CMD+=" --save-dir /results/format_aligned_ckpt"
CMD+=" --results-dir /results/format_aligned_final"
