...
"""Megatron-compatible attention module using Transformer Light kernels.

This module provides :class:`TransformerLightDotProductAttention`, a drop-in
...
class TransformerLightDotProductAttention(MegatronModule):
    ...
    Backends (``--tl-attn-backend``):
        * ``aiter``      – AITER flash-attention (default, fastest on MI300X)
        * ``triton``     – Triton flash-attention (always available)
        * ``triton_fp8`` – Triton FP8 quantised attention
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        args = get_args()
        self.backend = getattr(args, "tl_attn_backend", "aiter")
    ...
    def forward(
        ...
        if self.backend == "triton_fp8":
            ...
            if self.backend == "aiter" and not is_aiter_available():
                raise RuntimeError(
                    "AITER is not installed. The aiter backend "
                    "requires 'aiter' — install it or use "
                    "--tl-attn-backend triton."
                )
            out = attention(
                q, k, v,
                dropout_p=dropout_p,
