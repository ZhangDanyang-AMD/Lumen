...
from transformer_light.ops.quantize import convert_to_mxfp8
...
def block_scaling_node(tensor, use_fp8, BLOCK_M=FIXED_BLOCK_M, float8_dtype=get_f8_fwd_dtype()):
    """Scale *tensor* in per-block mode for FP8 blockwise attention."""
    if use_fp8:
...
def per_tensor_fp8_quantize(tensor, float8_dtype=None):
    """Per-tensor quantization to FP8.
    ...
    amax = torch.where(amax == 0, torch.tensor(dtype_max, device=tensor.device, dtype=tensor.dtype), amax)
...
if csrc_available("flash_attn_fwd"):
    ...
    @_torch_custom_op_wrapper(
        "transformer_light::attention_aiter_csrc_forward_impl",
...
if csrc_available("flash_attn_bwd"):
    ...
    @_torch_custom_op_wrapper(
        "transformer_light::attention_aiter_csrc_backward_impl",
...
if csrc_available("flash_attn_fp8_pertensor_fwd"):
    ...
    @_torch_custom_op_wrapper(
        "transformer_light::attention_aiter_csrc_fp8_pertensor_forward_impl",
        mutates_args=(), device_types="cuda",
    )
...
@_torch_custom_op_wrapper(
    "transformer_light::attention_triton_forward_impl",
    mutates_args=(), device_types="cuda",
)
...
@_torch_custom_op_wrapper(
    "transformer_light::attention_triton_backward_impl",
    mutates_args=("dq", "dk", "dv"), device_types="cuda",
)
...
@_torch_custom_op_wrapper(
    "transformer_light::attention_mxfp8_forward_triton_impl",
    mutates_args=(), device_types="cuda",
)
...
def attention_forward(q, k, v, use_fp8, dropout_p, softmax_scale, causal,
    ...
    if (
        ...
        q_fp8, q_descale = per_tensor_fp8_quantize(q, fp8_dt)
        k_fp8, k_descale = per_tensor_fp8_quantize(k, fp8_dt)
        v_fp8, v_descale = per_tensor_fp8_quantize(v, fp8_dt)
...
def attention_mxfp8_forward(q, k, v, use_mxfp8, dropout_p, softmax_scale,
    ...
    if use_mxfp8:
        ...
        q, q_scale = block_scaling_node_mxfp8(
            ...
            float8_dtype_pt=fp8_dt, cu_seqlens=0, max_seqlens=q.shape[1],
        )
        k, k_scale = block_scaling_node_mxfp8(
            k, quant_block_size, "bshd", is_2d_block=True,
            float8_dtype_pt=fp8_dt, cu_seqlens=0, max_seqlens=k.shape[1],
        )
