...
"""Manual tile autotuner for the implicit-GEMM conv3d kernel, vendored from FlyDSL.

Companion to ``conv3d_implicit.py``; see that module's header for why these live
in Lumen instead of coming from the flydsl wheel.

Upstream: FlyDSL ``kernels/conv/conv3d_autotune.py`` @ ROCm/FlyDSL#1069
(4c237b4), unmodified apart from this header.

Off by default -- ``conv3d_implicit`` only reaches this module when the caller
passes ``autotune=True`` or sets ``FLYDSL_CONV3D_AUTOTUNE=1``. Winners are cached
under ``FLYDSL_AUTOTUNE_CACHE_DIR`` (default ``~/.flydsl/autotune``)."""
