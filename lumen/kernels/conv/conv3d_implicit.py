...
"""Double-buffered implicit-GEMM conv3d (BF16), vendored from FlyDSL.

Lumen normally leaves GPU kernels to AITER and only dispatches. This one is an
exception, for a reason specific to how FlyDSL ships: the kernel lives in
FlyDSL's repo-level ``kernels/conv/``, and its ``setup.py`` packages ``python/``
only, so the wheel does not carry it. Depending on AITER to re-export it would
couple Lumen's conv support to an AITER release, so Lumen keeps its own copy and
talks to the installed ``flydsl`` compiler directly. Only published flydsl APIs
are used.

Upstream: FlyDSL ``kernels/conv/conv3d_implicit.py`` @ ROCm/FlyDSL#1069
(3333b562). Behavior matches upstream exactly. Both projects run black at line
length 120, so ``diff`` against upstream stays small and readable -- 36 lines,
all of them one of three things: this header, the ``conv3d_autotune`` import made
relative and ``buffer_atomic_add`` inlined (so ``kernels/common/mem_ops.py`` need
not come along), and one ``import torch`` moved because Lumen's isort treats
``flydsl`` as third-party while FlyDSL's own treats it as first-party. Keep it
that way and re-applying an upstream tile-heuristic change stays trivial.
...
def buffer_atomic_add(vdata, rsrc, offset, soffset, aux):
    ...
    Upstream keeps this in ``kernels/common/mem_ops.py`` because its MoE wrappers
    share it; the split-K epilogue below is the only caller here, so it is
    inlined rather than dragging that module in. Operates on a buffer resource +
    byte offset, not an ``!llvm.ptr``.
    """
...
def _conv3d_impl(
    ...
    if tile is not None:
        ...
        from .conv3d_autotune import BF16_CANDIDATES, WGM_VALUES, autotune_conv3d
