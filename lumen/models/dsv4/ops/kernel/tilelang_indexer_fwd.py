...
def indexer_fwd_interface(q, kv, weights, cu_seqlen_ks, cu_seqlen_ke, clean_logits=True):
    ...
    block_n = int(os.environ.get("V4_INDEXER_BLOCK_N", "256"))
    num_stages = int(os.environ.get("V4_INDEXER_NUM_STAGES", "3"))
    tl_indexer_fwd_kernel = tl_indexer_fwd_impl(
        heads=heads, index_dim=index_dim, block_N=block_n, num_stages=num_stages
    )
