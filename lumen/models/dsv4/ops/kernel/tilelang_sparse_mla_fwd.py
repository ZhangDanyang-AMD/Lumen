...
def sparse_mqa_fwd(
    ...
    if is_hip and _get_max_smem() <= 65536:
        max_H_per_block = 16
    else:
        max_H_per_block = 64

    if padded_H > max_H_per_block:
        assert padded_H % max_H_per_block == 0, f"padded heads ({padded_H}) must be a multiple of {max_H_per_block}"
        REPLICATE_H = padded_H // max_H_per_block
        H_per_block = max_H_per_block
    else:
        REPLICATE_H = 1
    ...
    def main(
        ...
        with T.Kernel(seq_len * REPLICATE_H, batch, threads=threads) as (bx, by):
            ...
            T.copy(Q_shared, Output[b_i, s_i, H0:H1, :])
...
def _get_max_smem():
    """Query max shared memory per block for the current HIP device."""
    try:
...
def sparse_mqa_fwd_interface(q, kv, attn_sink, topk_idxs, sm_scale=None, block_I=64, num_stages=2, threads=256):
    ...
    # gfx942 (MI300X/MI308X) has 64KB LDS vs gfx950 (MI350X) 128KB.
    # Halve block_I, single-stage pipeline, and match threads to avoid MFMA layout errors.
    if getattr(torch.version, "hip", None) and _get_max_smem() <= 65536:
        block_I = min(block_I, 32)
        num_stages = 1
