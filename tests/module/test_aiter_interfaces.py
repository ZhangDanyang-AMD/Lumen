...
###############################################################################
"""
Unit tests for AITER calling interfaces in lumen.

These tests use ``unittest.mock`` to verify that the correct AITER functions
are invoked with correct arguments when the ``"aiter_csrc"`` backend is selected.
...
Covers:
    ...
    5. Module wrappers: ``LumenAttention``,
       ``LumenLinear``
...
class TestIsAiterAvailable:
    """Tests for ``lumen.quantize.is_aiter_available``."""
...
class TestGetAttentionBackend:
    """Tests for ``lumen.quantize.get_attention_backend``."""

    def test_aiter_triton_explicit(self):
        from lumen.quantize import get_attention_backend

        assert get_attention_backend("aiter_triton") == "aiter_triton"

    def test_aiter_csrc_explicit_available(self):
        from lumen.quantize import get_attention_backend

        with patch("lumen.quantize.is_aiter_available", return_value=True):
            assert get_attention_backend("aiter_csrc") == "aiter_csrc"

    def test_aiter_csrc_explicit_unavailable_raises(self):
        from lumen.quantize import get_attention_backend

        with patch("lumen.quantize.is_aiter_available", return_value=False):
            with pytest.raises(RuntimeError, match="AITER is not installed"):
                get_attention_backend("aiter_csrc")

    def test_auto_with_aiter(self):
        from lumen.quantize import get_attention_backend

        with patch("lumen.quantize.is_aiter_available", return_value=True):
            assert get_attention_backend("auto") == "aiter_csrc"

    def test_auto_without_aiter(self):
        from lumen.quantize import get_attention_backend

        with patch("lumen.quantize.is_aiter_available", return_value=False):
            assert get_attention_backend("auto") == "aiter_triton"


class TestGetQuantBackend:
    """Tests for ``lumen.quantize.get_quant_backend``."""

    def test_triton_explicit(self):
        from lumen.quantize import get_quant_backend
...
class TestAttentionAiterInterface:
    ...
    def test_attention_calls_flash_attn_func_with_correct_kwargs(self):
        ...
        with patch("lumen.ops.attention.attention._is_aiter_available", return_value=True), patch(
            "lumen.ops.attention.attention.flash_attn_func", mock_flash, create=True
        ):
    ...
    def test_attention_passes_bias_and_alibi_slopes(self):
        ...
        with patch("lumen.ops.attention.attention._is_aiter_available", return_value=True), patch(
            ...
            attention(q, k, v, bias=bias, alibi_slopes=alibi, backend_type="aiter_csrc")
    ...
    def test_attention_aiter_raises_when_unavailable(self):
        with patch("lumen.ops.attention.attention._is_aiter_available", return_value=False):
            from lumen.ops.attention.attention import attention
    ...
    def test_attention_auto_selects_aiter_when_available(self):
        ...
        with patch("lumen.ops.attention.attention._is_aiter_available", return_value=True), patch(
            ...
            attention(torch.randn(B, S, H, D), torch.randn(B, S, H, D), torch.randn(B, S, H, D), backend_type="auto")
        mock_flash.assert_called_once()
    ...
    def test_attention_fp8_quant_rejects_aiter_csrc_backend(self):
...
class TestAttentionAiterCPA2AInterface:
    ...
    def test_aiter_cpa2a_calls_flash_attn_func(self):
        ...
        with patch("lumen.ops.attention.attention_with_cp_a2a.flash_attn_func", mock_flash, create=True), patch(
            "lumen.ops.attention.attention_with_cp_a2a.torch.distributed.all_to_all_single",
...
class TestQuantizedLinearAiterInterface:
    ...
    def test_forward_calls_aiter_quant_and_mm(self):
        ...
        with patch("lumen.ops.quantize.linear._aiter_quant", mock_quant), patch(
            "lumen.ops.quantize.linear._aiter_mm", mock_mm
        ), patch("lumen.ops.quantize.linear.is_aiter_available", return_value=True):
    ...
    def test_forward_aiter_raises_when_unavailable(self):
        ...
        with patch("lumen.ops.quantize.linear.is_aiter_available", return_value=False):
            with pytest.raises(RuntimeError, match="AITER is not installed"):
                quantized_linear(
                    torch.randn(2, 64),
                    torch.randn(32, 64),
    ...
    def test_aiter_quant_delegates_to_per_token_quant_hip(self):
        ...
        with patch.dict(
            ...
            from lumen.ops.quantize.linear import _aiter_quant
    ...
    def test_aiter_mm_delegates_to_hipb_mm(self):
        ...
        with patch.dict(
            ...
            from lumen.ops.quantize.linear import _aiter_mm
...
class TestRMSNormAiterInterface:
    ...
    def test_rmsnorm_calls_aiter_backend(self):
        ...
        with patch("lumen.ops.normalization.rmsnorm._USE_AITER", True), patch(
            "lumen.ops.normalization.rmsnorm._aiter_rms_norm", mock_rms
        ):
    ...
    def test_rmsnorm_falls_back_to_torch(self):
        ...
        with patch("lumen.ops.normalization.rmsnorm._USE_AITER", False):
            from lumen.ops.normalization.rmsnorm import rmsnorm
    ...
    def test_rmsnorm_module_delegates_to_functional(self):
        ...
        with patch("lumen.ops.normalization.rmsnorm._USE_AITER", True), patch(
            ...
            from lumen.ops.normalization.rmsnorm import LumenRMSNorm

            norm = LumenRMSNorm(hidden, eps=1e-5)
            norm(x)
        mock_rms.assert_called_once()
    ...
    def test_rmsnorm_module_extra_repr_with_grad_quant(self):
        ...
        norm = LumenRMSNorm(64, grad_quant_type="mxfp8")
        r = norm.extra_repr()
...
class TestLumenAttentionModule:
    """Tests for ``modules.attention.LumenAttention``."""

    def test_aiter_csrc_backend_requires_aiter(self):
        with patch("lumen.modules.attention.is_aiter_available", return_value=False):
            from lumen.modules.attention import LumenAttention

            with pytest.raises(RuntimeError, match="AITER is not installed"):
                LumenAttention(backend_type="aiter_csrc", quant_type=None)

    def test_aiter_csrc_backend_succeeds_when_available(self):
        with patch("lumen.modules.attention.is_aiter_available", return_value=True):
            from lumen.modules.attention import LumenAttention

            attn = LumenAttention(backend_type="aiter_csrc", quant_type=None)
            assert attn.backend_type == "aiter_csrc"

    def test_aiter_triton_backend_no_aiter_check(self):
        from lumen.modules.attention import LumenAttention

        attn = LumenAttention(backend_type="aiter_triton", quant_type=None)
        assert attn.backend_type == "aiter_triton"

    def test_aiter_csrc_plus_quant_type_raises(self):
        with patch("lumen.modules.attention.is_aiter_available", return_value=True):
            from lumen.modules.attention import LumenAttention
    ...
    def test_forward_passes_grad_quant_type(self):
        ...
        attn(torch.randn(2, 4, 8, 32), torch.randn(2, 4, 8, 32), torch.randn(2, 4, 8, 32))
    ...
    def test_attention_fn_is_attention_for_aiter_csrc(self):
        """When backend_type='aiter_csrc', attention_fn should be the plain
    ...
    def test_attention_fn_is_fp8_quant_for_aiter_triton(self):
        """When backend_type='aiter_triton', attention_fn should be
        ``attention_fp8_quant``."""
        from lumen.modules.attention import LumenAttention
        from lumen.ops.attention import attention_fp8_quant
...
class TestLumenLinearModule:
    """Tests for ``modules.quantize.LumenLinear``."""

    def test_aiter_backend_requires_aiter(self):
        with patch("lumen.modules.quantize.is_aiter_available", return_value=False):
            from lumen.modules.quantize import LumenLinear

            with pytest.raises(RuntimeError, match="AITER is not installed"):
                LumenLinear(64, 32, backend_type="aiter")

    def test_aiter_backend_succeeds_when_available(self):
        with patch("lumen.modules.quantize.is_aiter_available", return_value=True):
            from lumen.modules.quantize import LumenLinear

            linear = LumenLinear(64, 32, backend_type="aiter")
    ...
    def test_triton_backend_no_aiter_check(self):
        ...
        linear = LumenLinear(64, 32, backend_type="triton")
        assert linear.backend_type == "triton"

    def test_bias_creation(self):
        from lumen.modules.quantize import LumenLinear

        with_bias = LumenLinear(64, 32, backend_type="triton", bias=True)
        assert with_bias.bias is not None
        assert with_bias.bias.shape == (32,)

        no_bias = LumenLinear(64, 32, backend_type="triton", bias=False)
    ...
    def test_forward_delegates_to_quantized_linear(self):
        ...
        linear = LumenLinear(128, 64, backend_type="triton")
        mock_ql = MagicMock(return_value=torch.randn(2, 64))
        with patch("lumen.modules.quantize.quantized_linear", mock_ql):
...
class TestQuantEnable:
    """Tests for ``lumen.quantize.enable()`` /
    ``lumen.quantize.disable()``."""
    ...
    def test_enable_patches_all_linear_layers(self):
        ...
        with patch("lumen.quantize.get_quant_backend", return_value="triton"):
    ...
    def test_enable_sets_unique_tensor_ids(self):
        ...
        ids = [m._quant_tensor_id for m in model.modules() if isinstance(m, nn.Linear)]
    ...
    def test_enable_with_aiter_backend(self):
        ...
        model = self._make_model()
        with patch("lumen.quantize.is_aiter_available", return_value=True), patch(
            "lumen.quantize.get_quant_backend", return_value="aiter"
        ):
            quant.enable(model, format="fp8_e4m3", scaling="dynamic", backend="aiter")
    ...
    def test_enable_with_triton_backend(self):
        ...
        model = self._make_model()
        with patch("lumen.quantize.get_quant_backend", return_value="triton"):
            quant.enable(model, format="fp8_e4m3", scaling="dynamic", backend="triton")
...
class TestGradQuantUtility:
    ...
    def test_scaling_manager_quantize_grad_fp8(self):
        ...
        with patch("lumen.quantize.scaling_manager._round_to_fp8", return_value=expected) as mock_round:
    ...
    def test_scaling_manager_quantize_grad_mxfp8(self):
        ...
        t = torch.randn(4, 8)
        expected = torch.randn(4, 8)
        with patch("lumen.quantize.scaling_manager._round_to_mxfp8", return_value=expected) as mock_round:
            result = mgr.quantize_grad(t)
    ...
    def test_static_quantize_grad_tensor_fp8(self):
        ...
        t = torch.randn(4, 8)
        expected = torch.randn(4, 8)
        with patch("lumen.quantize.scaling_manager._round_to_fp8", return_value=expected):
            result = ScalingManager.quantize_grad_tensor(t, "fp8", fp8_dtype=torch.float8_e4m3fn)
        assert result is expected
    ...
    def test_static_quantize_grad_tensor_auto_dtype(self):
        ...
        t = torch.randn(4, 8)
        with patch("lumen.quantize.scaling_manager._round_to_fp8", return_value=t) as mock_round, patch(
            "lumen.quantize.scaling_manager._get_float8_e4m3", return_value=torch.float8_e4m3fn
        ):
    ...
    def test_grad_quant_types_in_scaling_manager(self):
        from lumen.quantize.scaling_manager import GRAD_QUANT_TYPES
    ...
    def test_grad_quant_types_backward_compat(self):
        from lumen.core.grad_quant import GRAD_QUANT_TYPES
...
class TestAttentionQuantPrimitives:
    ...
    def test_quantize_per_tensor_fp8_auto_dtype(self):
        ...
        t = torch.randn(4, 8)
        with patch("lumen.quantize.scaling_manager._get_float8_e4m3", return_value=torch.float8_e4m3fn):
    ...
    def test_quantize_block_mxfp8_bshd(self):
        ...
        with patch("lumen.quantize.scaling_manager.convert_to_mxfp8", mock_quant):
    ...
    def test_attention_forward_fp8_uses_scaling_manager_block(self):
        ...
        block_result = (torch.randn(2, 128, 8, 64).to(torch.float8_e4m3fn), torch.randn(2, 8, 2))
        v_result = (torch.randn(2, 128, 8, 64).to(torch.float8_e4m3fn), torch.tensor(1.0), 448.0)
        with patch.object(ScalingManager, "quantize_block_fp8", return_value=block_result) as mock_block, patch.object(
            ScalingManager, "quantize_v_fp8", return_value=v_result
        ) as mock_v:
    ...
    def test_attention_forward_pertensor_uses_scaling_manager(self):
        ...
        t = torch.randn(2, 128, 8, 64)
        expected = (torch.randn(2, 128, 8, 64).to(torch.float8_e4m3fn), torch.tensor([0.5]))
        with patch.object(ScalingManager, "quantize_per_tensor_fp8", return_value=expected) as mock_fn:
...
class TestAPISignatures:
    ...
    def test_lumen_attention_init_signature(self):
        from lumen.modules.attention import LumenAttention

        sig = inspect.signature(LumenAttention.__init__)
        params = sig.parameters
        assert "backend_type" in params
        assert "quant_type" in params
        assert "grad_quant_type" in params
        assert params["backend_type"].default == "aiter_csrc"

    def test_lumen_rmsnorm_init_signature(self):
        from lumen.ops.normalization.rmsnorm import LumenRMSNorm

        sig = inspect.signature(LumenRMSNorm.__init__)
        params = sig.parameters
