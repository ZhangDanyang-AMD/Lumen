...
class TestAttentionAiterInterface:
    ...
    def test_attention_calls_flash_attn_func_with_correct_kwargs(self):
        ...
        with patch("lumen.ops.attention.attention._is_aiter_available",
                    return_value=True), \
             patch("lumen.ops.attention.attention.flash_attn_func",
    ...
    def test_attention_return_lse_forwarded(self):
        ...
        with patch("lumen.ops.attention.attention._is_aiter_available",
            ...
            attention(torch.randn(B, S, H, D),
                      torch.randn(B, S, H, D),
                      torch.randn(B, S, H, D),
                      return_lse=True, backend_type="aiter_csrc")
    ...
    def test_attention_accepts_grad_quant_type(self):
        ...
        with patch("lumen.ops.attention.attention._is_aiter_available",
            ...
            attention(torch.randn(B, S, H, D),
                      torch.randn(B, S, H, D),
                      torch.randn(B, S, H, D),
                      backend_type="aiter_csrc", grad_quant_type="fp8")
        mock_flash.assert_called_once()
...
class TestAttentionAiterCPA2AInterface:
    ...
    def test_aiter_cpa2a_calls_flash_attn_func(self):
        ...
        with patch("lumen.ops.attention.attention_with_cp_a2a.flash_attn_func",
                    mock_flash, create=True), \
             patch("lumen.ops.attention.attention_with_cp_a2a.torch.distributed.all_to_all_single",
...
class TestQuantizedLinearAiterInterface:
    ...
    def test_forward_calls_aiter_quant_and_mm(self):
        ...
        with patch("lumen.ops.quantize.linear._aiter_quant", mock_quant), \
             patch("lumen.ops.quantize.linear._aiter_mm", mock_mm), \
             patch("lumen.ops.quantize.linear.is_aiter_available",
                    return_value=True):
    ...
    def test_forward_aiter_raises_when_unavailable(self):
        from lumen.ops.quantize.linear import quantized_linear
        with patch("lumen.ops.quantize.linear.is_aiter_available",
                    return_value=False):
            with pytest.raises(RuntimeError, match="AITER is not installed"):
                quantized_linear(
                    torch.randn(2, 64),
...
class TestRMSNormAiterInterface:
    ...
    def test_rmsnorm_calls_aiter_backend(self):
        ...
        with patch("lumen.ops.normalization.rmsnorm._USE_AITER", True), \
             patch("lumen.ops.normalization.rmsnorm._aiter_rms_norm", mock_rms):
...
class TestLumenAttentionModule:
    ...
    def test_aiter_csrc_backend_requires_aiter(self):
        with patch("lumen.modules.attention.is_aiter_available",
                    return_value=False):
    ...
    def test_aiter_csrc_plus_quant_type_raises(self):
        with patch("lumen.modules.attention.is_aiter_available",
            ...
            with pytest.raises(AssertionError):
                LumenAttention(
                    backend_type="aiter_csrc", quant_type="fp8_blockwise",
                )

    def test_grad_quant_type_stored(self):
        from lumen.modules.attention import LumenAttention
        attn = LumenAttention(
            backend_type="aiter_triton", quant_type="fp8_blockwise",
...
class TestLumenLinearModule:
    ...
    def test_aiter_backend_requires_aiter(self):
        with patch("lumen.modules.quantize.is_aiter_available",
                    return_value=False):
...
class TestQuantEnable:
    ...
    def test_enable_patches_all_linear_layers(self):
        ...
        with patch("lumen.quantize.get_quant_backend",
                    return_value="triton"):
    ...
    def test_enable_with_aiter_backend(self):
        import lumen.quantize as quant
        model = self._make_model()
        with patch("lumen.quantize.is_aiter_available",
                    return_value=True), \
             patch("lumen.quantize.get_quant_backend",
                    return_value="aiter"):
            quant.enable(model, format="fp8_e4m3", scaling="dynamic",
                         backend="aiter")
...
class TestGradQuantUtility:
    ...
    def test_scaling_manager_quantize_grad_none_is_noop(self):
        from lumen.quantize import ScalingManager, QuantConfig
    ...
    def test_scaling_manager_quantize_grad_fp8(self):
        ...
        with patch("lumen.quantize.scaling_manager._round_to_fp8",
    ...
    def test_scaling_manager_quantize_grad_mxfp8(self):
        ...
        t = torch.randn(4, 8)
        expected = torch.randn(4, 8)
        with patch("lumen.quantize.scaling_manager._round_to_mxfp8",
                    return_value=expected) as mock_round:
            result = mgr.quantize_grad(t)
    ...
    def test_static_quantize_grad_tensor_auto_dtype(self):
        ...
        with patch("lumen.quantize.scaling_manager._round_to_fp8",
             ...
             patch("lumen.quantize.scaling_manager._get_float8_e4m3",
...
class TestAttentionQuantPrimitives:
    ...
    def test_quantize_per_tensor_fp8_auto_dtype(self):
        from lumen.quantize import ScalingManager
        t = torch.randn(4, 8)
        with patch("lumen.quantize.scaling_manager._get_float8_e4m3",
                    return_value=torch.float8_e4m3fn):
    ...
    def test_quantize_block_mxfp8_bshd(self):
        ...
        with patch("lumen.quantize.scaling_manager.convert_to_mxfp8",
                    mock_quant):
