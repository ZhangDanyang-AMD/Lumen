...
Tests collective and P2P operations that training (Megatron/FSDP) relies on.
Designed to be launched with torchrun:

    # Single-node, all 8 GPUs:
    torchrun --nproc_per_node=8 tests/env/test_nccl.py

    # Quick single-GPU smoke test (verifies ROCm stack only):
    torchrun --nproc_per_node=1 tests/env/test_nccl.py

    # With extra RCCL diagnostics:
    NCCL_DEBUG=INFO torchrun --nproc_per_node=8 tests/env/test_nccl.py
...
def init_distributed() -> bool:
    ...
    # Bind device BEFORE init_process_group so RCCL knows which GPU owns this rank.
    ...
    try:
        ...
        # Older PyTorch builds may not accept device_id kwarg.
        try:
...
def test_p2p() -> None:
    ...
    try:
        ...
        world = _world()
        send_val = float(rank)
        send_t = _alloc(256).fill_(send_val)
...
def bench_allreduce_bandwidth() -> None:
    ...
    log(f"  {N} iterations in {elapsed:.3f}s  →  {bw_gb_s:.1f} GB/s (bus bandwidth)")
...
def test_dtype_collectives() -> None:
    ...
    for dtype, name in [(torch.bfloat16, "bf16"), (torch.float16, "fp16")]:
        try:
            ...
            dist.all_reduce(t, op=dist.ReduceOp.SUM)
            expected = float(_world())
            ok = torch.allclose(t.float(), torch.full((1024,), expected))
...
def main() -> int:
    # --- Phase 1: environment (no dist required) ---
    check_environment()

    # --- Phase 2: distributed init ---
    ok = init_distributed()
    if not ok:
        log("\n[rank0] Cannot continue — distributed init failed.", error=True)
        _print_summary()
        return 1
